...
#ifndef MULTIMAP_HASHMAP_H
#define MULTIMAP_HASHMAP_H
...
public:
    ...
    iterator end() {
        ...
        return iterator(prev, table, indices);
    }
...
public:
    ...
    std::pair<HashMap<K, V, F>::iterator, HashMap<K, V, F>::iterator> equal_range(const K &key) {
        ...
        HashNode<K, V> *lNode = node;
        while(lNode->getNext() != NULL) {
            lNode = lNode->getNext();
        }
        if (node != NULL) {
            iterator it1(node, table, indices);
            iterator it2(lNode, table, indices);
            return std::make_pair(it1, it2);
        } else {
            return std::make_pair(iterator(lNode, table, indices), iterator(lNode, table, indices));
        }
    }
};
